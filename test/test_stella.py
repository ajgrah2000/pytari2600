import unittest
import graphics.pygamestella
import graphics.stella
import clocks
import inputs

class TestCollisions(unittest.TestCase):
    def test_collisions(self):
        collisions = graphics.stella.CollisionState()
    ...
    def test_colors(self):
        """ Test color lookup. """
        c = graphics.pygamestella.PygameColors()
...
class TestDrawing(unittest.TestCase):
    ...
    def reference_update_playfield(self, pf0, pf1, pf2, ctrlpf, x):
        drawn = False
        if (x>=0) and (x < graphics.stella.Stella.FRAME_WIDTH & 0xFFFF):
    ...
    def reference_update_player(self, nusiz, grp, refp,  resp, x):
        ...
        if 0 != grp:
            (number, size, gap) = graphics.stella.Stella.nusize(nusiz)

            if resp < graphics.stella.Stella.HORIZONTAL_BLANK:
                resp = graphics.stella.Stella.HORIZONTAL_BLANK
    ...
    def test_update_player(self):
        ...
        s = graphics.stella.PlayerState(test_clocks)
    ...
    def test_update_playfield(self):
        test_clocks = clocks.Clock()
        test_input = inputs.Input()
        s = graphics.stella.PlayfieldState()
